...
## 2026-06-01
- style(pc-vue): footer 文字居中
- fix(pc-vue): 分类弹层移出 cate + hover 延迟防闪烁
- style(pc-vue): 全局 CSS reset + side 面板高度优化
...
- feat(pc-vue): HomeListView 分类 hover 弹出子分类弹层 + 全局滚动条美化
- feat(pc-vue): HomeListView Banner 左侧分类接入真实 API
- data: 修复 16 个一级分类名 + 5 个子分类名瑕疵
- feat: PC 前端接入分类树接口 + Gateway CORS 修复
- build: Jenkins JDK 17→21 升级（env-contain-install.py）
- build: JDK 17→21 升级（pom.xml + 全部 Dockerfile）
...
- feat: 插件化架构（中间件按需加载，@ConditionalOnProperty 条件注入）
- refactor: 插件化架构 - 中间件按需加载 (4 Plugin + EnvironmentPostProcessor + optional依赖)
