      ...
      <template #search>
        <el-form :inline="true">
          <el-form-item label="标题">
            <el-input v-model="searchObj.title" placeholder="快报标题" @keyup.enter="handleSearch" />
          </el-form-item>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="500px" :close-on-click-modal="false">
      <el-form :model="form" label-width="100px">
        <el-form-item label="标签">
          <el-input v-model="form.tag" placeholder="[特惠]/[公告]" />
        </el-form-item>
        <el-form-item label="标题">
          <el-input v-model="form.title" placeholder="快报标题" />
        </el-form-item>
...
import api from '@/api/cms/newsApi.js'
...
const initForm = { tag: '', title: '', linkUrl: '', sort: 0, status: 1 }
const form = reactive({ ...initForm })

const columns = [
  { prop: 'id', label: 'ID', width: 60 },
  { prop: 'tag', label: '标签', width: 80 },
...
const remove = (row) => {
  ElMessageBox.confirm(`确定删除快报【${row.title}】？`, '提示', {
